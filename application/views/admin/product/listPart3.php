...
$SQL = "SELECT a.id FROM invheader a, clients b 
		WHERE a.client_id=b.client_id 
		and id LIKE concat('%%', '" .$searchId. "')
		ORDER BY "
		 . $sidx . " " . $sord . " LIMIT " . $start . "," . $limit;

$sql = "SELECT mdl_cd id, part_ver, part_cd, part_nm, unit_price, unit_wgt, remark, srl_no, recom_yn, use_yn, disp_yn, ord_num";
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
	...
	$responce['rows'][$i]['invdate'] = $row['id'];
	$responce['rows'][$i]['name'] = $row['part_nm'];
	$responce['rows'][$i]['amount'] = $row['id'];
	$responce['rows'][$i]['tax'] = $row['id'];
	$responce['rows'][$i]['rate_end_date'] = $row['id'];
	$responce['rows'][$i]['total'] = $row['id'];
	$responce['rows'][$i]['note'] = $row['id'];
#    echo $row['id'];
    $i++;
}  
